...
interface ProcedureCodeFormModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (code: NewProcedureCode) => void;
  initialCode?: ProcedureCode;
  isEditing?: boolean;
}

const defaultCode: NewProcedureCode = {
  code: "",
  description: "",
  category: "",
  version: "CPT",
...
const ProcedureCodeFormModal: React.FC<ProcedureCodeFormModalProps> = ({
  isOpen,
  onClose,
  onSave,
  initialCode,
  ...
  const [formData, setFormData] = useState<NewProcedureCode>(defaultCode);
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.defaultDuration && formData.defaultDuration <= 0) {
      newErrors.defaultDuration = "Duration must be greater than zero";
    }
  ...
  const versionOptions = ["CPT", "HCPCS", "ICD-10-PCS"];
...
export default ProcedureCodeFormModal;
